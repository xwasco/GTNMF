function A = sv_scale_dist3_knn(D, nn, knn, useSparse)
%SCALE_DIST3_KNN
...
if numel(knn)>1
    mask=zeros(size(D,1));
    %A=inf(size(D,1));
    s=zeros(size(D,1),1)+eps;
    [V,IDX]=sort(D,2);    
    for i=1:numel(knn)
        %find the k neighbours of each node
        %[v, idx]=sort(D(i,:));
        s(i)=sqrt(V(i,nn+1)); %compute sigmas
        id=IDX(i,2:knn(i)+1);
        mask(i,id)=1; %v(2:knn(i)+1); 
    end
    mask=(mask+mask')>0;
    
    A=exp(-D./(s*s')).*not(eye(size(mask))).*mask;
    A=(A+A').*0.5;
else
    if (distSparse)
